...
- 形式化地讲，一个 general model-free RL framwork 大概会是下面这样
	...
	使用流程图表示这个过程
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/16e53d70b9f39fd4c8e52418d2acccbb.png#pic_center =85%x)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0ab3227fe74b121d7d75249db9488c7d.png#pic_center =85%x)


- 再次精炼这个过程，只看最重要的部分，其实强化学习做的事情就是 **利用奖励估计策略价值，再根据价值改进策略**
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/aee6cd4c8beffa6f37e7309515b20976.png#pic_center =80%x)
...
- 根据前文分析，有 $G_t = r_{t+1}+\gamma G_{t+1}$，注意价值函数是对期望收益 $\mathbb{E}[G]$ 的估计，这启发我们在做 prediction时，使用当前计算的下一时刻 $s'$ 或 $(s',a')$ 的价值 $v_\pi(s')$ 或 $q_\pi(s',a')$ 代替 $G_{t+1}$。也就是说
	...
	这个想法是合理的，以状态价值为例，**$v_\pi(s)$ 和 $r_{t+1}+\gamma v_\pi(s')$ 都是当前对真实价值的估计**，有 $$\mathbb{E}[v_\pi(s)] = \mathbb{E}[r_{t+1}+\gamma v_\pi(s')]$$ **后者包含一个真实得到的奖励 $r_{t+1}$，因此比纯粹估计更准确一些，应当让估计朝着向它靠近的方向更新**，我们称要靠近的目标为 `TD target`，称更新时的差值为 `TD error`
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/97c98df9b567898a41683027fa1952c4.png#pic_center =50%x)
	2. 对比看 sarsa 和 policy evaluation；Q-learning 和 policy iteration，你会发现 **TD 就是在利用 MC 的方式，使价值函数匹配 Bellman Equation 和  Bellman Optimal Equation**。以 sarsa 对 $V$ 的评估为例进行说明
		1. 针对非平稳情况的增量式 MC 为：$$v_\pi(s) \leftarrow v_\pi(s) + \alpha(g_t-v_\pi(s)), \alpha \in [0,1]$$ 估计结果为 $$v_\pi(s_t) = \mathbb{E}_\pi[g_t]$$
		2. Sarsa 更新公式为 $$v_\pi(s) \leftarrow v_\pi(s) + \alpha\big[r_{t+1}+\gamma v_\pi(s')-v_\pi(s)\big], \alpha \in [0,1]$$ 发现和上面的形式完全相同，估计结果为 $$v_\pi(s_t) = \mathbb{E}_{\pi}[R_{t+1} + \gamma v_{\pi}(S_{t+1})| s]$$ 这正是 Bellman equation，因此 **RL 中的 TD 本质就是一种以匹配 Bellman equation 或 Bellman optimal equation 为目标的非平稳增量式 MC**

	所以后面我们一旦看到 TD，就要知道这个方法中也使用 MC 思想了
	>这里我想补充一点，如果**从 agent 获取指导信息的角度来考虑，可以发现 TD 方法的一个重要特点**：
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/24cafe5cd9f5e1de70d477d9a2b0a07b.png#pic_center)
...
- 当引入函数近似时，上述问题更加严重，我们知道函数近似方法有两个特点：“计算越多的位置越准确” 和 “有相似特征的位置会受到泛化性影响”，看以下示例
	![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/6f53a9ab03d45da9bc02dd72b809bd45.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/ded0122604cb49299ee45092855bad89.png#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f6074af115154ecfc99f04ffad4faf09.png#pic_center =60%x)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/30618976cbaa3e50905f62985db8ebd5.gif#pic_center =60%x)
## 4.3 结果分析
...
- 总的来说，样本效率是一个 “低-高-低” 的变化规律，形如下面的钟形曲线所示
	![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/1f925bd90c88b96fb9fe725d68fbb8dc.png#pic_center =70%x)
...
- 对于一些极端情况，问题会更加严重
	...
	![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/02284e63011915882d89fcf7fa3b9b51.gif =40%x)![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/76020ea42b9c7568892b531b7acfaec9.gif =40%x)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/c9a1cbe812de831c70c4cc0c45d62254.png#pic_center =95%x)
再放一张图片配合总结下（这是我之前一个 presentation 的 PPT，以后可能会讲一下发 B 站）
	![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/856cc27a50472e19455313b772050750.png#pic_center =88%x)
...
- 最后跳出一般 model-free RL 的框架打开思路看看
	...
	![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/0dca8ab691af0f51fff541eaa9ac06bb.png#pic_center =90%x)
...
- 针对片面探索的问题，下面展示两个最简单的缓解方法
	1. **使用非平稳 $\varepsilon$-greedy 探索策略**，下图显示 1000 episodes，随机动作概率 $\varepsilon$ 从1均匀下降到0。这种方法可以在早期探索尽量多的区域，并在片面探索发生之前完成价值传递
		![在这里插入图片描述](https://img-blog.csdnimg.cn/837f148f9df84679aef4c7c9c8abbf2e.gif#pic_center#pic_center =70%x)
	...
	2. **增加一个 experience replay buffer**，对于 off-policy 方法，可以把过去发生的转移四元组拿出来重复计算。早期的盲目探索阶段我们收集了很多转移，由于价值尚未传播开，当时看来可能很多转移都是 TD error 为 0，但随着价值估计逐渐准确，许多相同的转移对应的 TD error 很可能不为 0 了。 下图显示 500 episodes，每两个 step 间利用 replay 数据更新 5 次
		![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/9eea0a7acf4abbbcb32dcbae3a6690aa.gif#pic_center =70%x)
