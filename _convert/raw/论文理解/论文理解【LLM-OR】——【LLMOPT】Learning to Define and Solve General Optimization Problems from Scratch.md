...
- 本文提出 LLMOPT 框架，致力于提升模型的求解精度和通用性。如下图所示
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/821b8a0bc6be452a8dbdf730ad07e0cf.png#pic_center =80%x)
...
- 作者提出基于优化问题的 5 个关键部分：**`集合Sets`、`参数Parameters`、`变量Variables`、`目标Objective`与`约束Constraints`** 定义优化问题的通用描述形式
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/2b1cb9457a1742c884311a572be70e2d.png#pic_centef =90%x)
...
- 现有数据集大多缺少五要素表达和求解代码标签，需进行数据增强，本文数据增强与标注流程如下
	1. 首先收集现有 OR 任务数据集，从每个数据集中随机抽取 100 个样本作为保留测试集（尺寸小于 100 的数据集全部用于测试），其余用于数据增强
		![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/5bb9a46971e341ddbf1fd5f60836c48a.png#pic_center =85%x)
	2. 使用 LLM 对原始问题进行扩展，生成的问题交由人类专家进行审查，移除描述不清或解不可行的问题，以确保数据多样性与质量
		> 本文对 1,763 个种子问题应用了 7 类不同指令，从多个角度对原问题进行扩展，例如修改约束、改变场景、改变优化类型、以及从原问题分叉生成新问题等。Prompt 模板如下
			![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/8d710c405314499d96402422232e1cb7.png#pic_center =85%x)
		> 数据增强规则如下
		![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/1114efe619b245f09713fe7ca0d21ebd.png#pic_center =85%x)
	3. 如下图所示，人类专家和 GPT4 模型分别对每个问题填写五要素建模标签 $f, f'$ 和求解代码标签 $s,s'$。由于 GPT-4 可能产生错误，专家会对 $f',s'$ 进行验证，并给出一个可取性（desirability）标签 $d'$ 来指示 GPT-4 的标签是否准确（True/False）
		![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/f8e44eb1985e4114a95bd9578e0f5450.png#pic_center =50%x)
...
- LLMOPT 的模型训练包含 SFT 和基于 KTO 的人类偏好对齐两个环节，如下图所示
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/eba2baf074b2441dafdbd1799620e243.png#pic_center =70%x)
...
- 监督微调阶段，直接把两类数据集混合 $D_{\mathrm{SFT}}=D_{\mathrm{SFT}}^{f} \cup D_{\mathrm{SFT}}^{s}$，然后最大化条件概率 $p(v|u)$，优化以下负对数似然损失
	$$L_{\mathrm{SFT}}(\theta)=-\mathbb{E}_{(u, v) \sim D_{\mathrm{SFT}}} \sum_{i=1} \log \pi\left(v_{i} \mid v_{0: i-1}, u ; \theta\right)$$ 同时优化模型 “定义与求解” 两方面的能力
...
- 基于本文的数据标注形式，作者使用 KTO (Kahneman-Tversky Optimization) 作为对齐方法。其使用的数据格式为 $$D_{\text{KTO}}=\{(u_i,v_i,d_i)\}_{i=1}^{N_{\text{KTO}}}$$ 其中 $u$ 是指令提示，$v$ 是 GPT4 生成的模型补全内容，$d\in\{\text{True, False}\}$ 是二元正确性标签
	...
	> 
	> 下面逐步来理解这一点
	> 1. 先看 KTO 在优化什么量。首先 alignment 类方法的优化目标可以转换成类似 RL 的收益最大化形式。对于 KTO 来说，可将对齐目标写成基于对数概率比的得分 $$r_{\mathrm{KTO}}(u, v)=\beta \log \frac{\pi_\theta(v \mid u)}{\pi_{\mathrm{ref}}(v \mid u)}$$ 这里 $\pi_{\text{ref}}$ 是参考模型（SFT 模型），$\pi_\theta$ 是当前模型。直观上：如果某个 completion $v$ 在 $\pi_\theta$ 下比在 $\pi_{\text{ref}}$ 下更 “被偏好”（概率更大），那 $\log \frac{\pi_\theta}{\pi_{\text{ref}}}$ 为正（对应正奖励），反之为负。它**衡量了当前模型 $\pi_\theta$ 相对参考模型 $\pi_{\text{ref}}$ 有多偏好 completion $v$**
...
- 自动测试流程自动化了 “问题定义 + 求解器代码生成” 的完整工作流，并集成自我纠错机制以实现持续改进，流程如下：
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/ce13e1baf9e148b985b6ec2c55439a26.png)
	...
	3. 分析求解器运行日志（包括输出结果与错误）以判断是否需要自我纠错。这里使用 prompt 模板控制 LLM 判断是否需要进一步处理。若需要处理，模型会生成带建议的分析，并决定返回到 “问题表述步骤” 还是 “代码生成步骤”，确保优化流程更鲁棒、更自适应，提升表述与最终解的准确性，从而改进优化泛化能力
		> 问题分析 prompt 模板如下
		![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/9bf0f6f0bfa5481caff4be4ede5f589e.png#pic_center =85%x)
# 3. 实验
...
- **【Q1】学习式 vs. 提示式**：LLMOPT 在 6 个数据集上以更少的求解次数取得更高的求解准确率，优于 GPT-4o 与 GPT-4-Turbo，体现了 Learning-based 方法的潜力
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/5049907544ef4151b65b3a0680959a8c.png#pic_center =85%x)
	此外，在 14B 模型上，仅 SFT 即可达到 GPT-4o 的水平
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/9d27386466604c1fb65211e377d52686.png#pic_center =80%x)
 - **【Q2-1】准确性**：**LLMOPT 在 6 个数据集上均取得 SA 的 SOTA**。相较学习式方法，五要素定义与自纠错流程提升准确性，在 4 个数据集上平均提升 14.83%；对提示式方法而言，SFT 与模型对齐增强了求解能力，在 3 个数据集上平均提升 10.67%。总体上，LLMOPT 在 6 个数据集上的 SOTA 平均提升为 11.08%
 	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/2c5b19a448984dc9a270c418251408bb.png#pic_center =85%x)
 	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/cf719fb67f3d436fa41ad8be5b3c38a1.png#pic_center =85%x)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b1dc510b55ae49d4a61bae8348e12289.png#pic_center =85%x)			
...
- **【Q3】问题定义的重要性**：**五要素问题定义能提升 6 个数据集上的 SA。但该定义有时会降低 ER**：没有五要素时，LLM 可能“过度简化”问题，从而生成无错误但不准确的代码；相反，使用五要素能保证生成更正确的代码，以略微降低 ER 为代价提升 SA
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/02732e73fdf041a6a75b69588f35cc6b.png#pic_center =85%x)
		![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/7955b04cfc344c08bfa64fa5c0dc6daf.png#pic_center =80%x)
- **【Q4】模型对齐的有效性**：**对齐通常提升 LLM 在特定任务上的效率与效果**。KTO 对齐不仅显著提升 SA，也在 6 个数据集上降低 AST
		![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/cbdae8a521074f7d885e95cb78494073.png#pic_center =75%x)进一步地，五要素问题定义与 KTO 的组合能在复杂数据集上同时改进 SA 与 AST
		![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/8c57fa9923ad4f0ba125303bc9d6d624.png#pic_center =75%x)
		![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/0c07ec6adea74cfe8c7101137a22c8ad.png#pic_center =75%x)
## 3.4 讨论
1.  **更大的基础模型**：如图所示，使用 72B 的基础模型可以进一步提升 SA 和 ER，但训练/部署成本高
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/f5115aa90d2e4b9c80f2f91d63ce12f0.png#pic_center =75%x)
...
3. **LLM 的 “跷跷板问题”（Seesaw Issue）**：**提升模型在专门任务上的性能与其在多样任务上的泛化之间存在权衡，即某一方面的收益往往会导致另一方面的损失**。为评估增强 LLM 定义与求解优化问题的能力是否会影响其在其他任务上的表现，作者比较了模型在微调前后、10 个通用任务上的性能，包括：数学、代码、分类、信息抽取、开放式问答、封闭式问答、文本生成、头脑风暴、改写与摘要
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/268feeb046f94cbf82e16c379e06bb69.png#pic_center =95%x)结果显示：微调后的模型在 6 个任务上性能提升、在 4 个任务上性能下降；在所有任务上的平均性能提升为 0.3%。重要的是，**LLMOPT 训练后并未观察到显著的权衡或跷跷板效应**
...
- 本文提出了 LLMOPT 框架，相较于传统方法，主要有以下特点
	1. **定义了通用的优化问题五要素表述作为数学建模形式**。作者认为这种通用表达可以避免 LLM 过度简化问题，有效提取潜在约束。
		...
		- 缺点在于构造数学建模标签人工成本高昂
	3. **引入 KTO 进一步实现人类偏好对齐**，这个确实在早期的 Learning-based 类方法中没有，这一点我觉得还是合理的，因为 LLM-OR 这个任务相当困难，很难把 SFT 做到极致，而**在数据质量低的情况下幻觉更容易产生，这时引入偏好对齐确实能有效提升性能**。不过这个思路也可以进一步发展，因为 OR 问题的求解正确性是可以验证的，所以直接上 RL 或许更有效，参考 [SIRL](https://blog.csdn.net/wxc971231/article/details/157909440)