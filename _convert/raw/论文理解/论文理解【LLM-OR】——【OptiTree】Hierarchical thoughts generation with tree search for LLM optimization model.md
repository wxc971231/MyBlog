...
- 作者在 IndustryOR 数据集上对传统 prompt-based 方法进行了分析，该数据集被划分为 Easy, Medium, Hard 三类难度水平；传统方法通常基于 CoT，通过顺序完成 “优化变量生成”、“约束条件设定” 和 “目标函数构建” 等分解步骤实现问题建模
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/430d8da7535f4f24a8fc19284efbd23b.png#pic_center =85%x)
...
- 基于上述观察，作者提出利用树搜索进行子问题分解，并利用子问题建模思路增强建模过程
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/2cf28d61f06548c68cf3faa55a0c7968.png#pic_center =90%x)
# 2. 本文方法
- 如下图所示，OptiTree 的核心思想包括
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/f1fa4e4ac9864dfb9f9d2f0385f35402.png#pic_center =90%x)
...
## 2.1 子问题的定义与识别
- 作者**通过问题建模定义子问题**，给定一个 OR 问题 $P$，设其形式化表示为 $$\min _{x} f(x) \quad \text { s.t. } \quad g_{i}\left(x ; \beta_{i}\right) \leq 0,  \text { for } i=1, \cdots, N \tag{1}$$ 其中 $f$ 表示目标函数，$x$ 为决策变量，$g_i$ 表示第 $i$ 个约束函数，$\beta_i$ 表示约束中的参数。假设决策变量可划分为两组 $x=\left(\tilde{x}_{1}, \tilde{x}_{2}\right)$，且目标与约束可分解为
	$$
	\begin{aligned}
	...
	\end{aligned}
	$$ 若另一个 OR 问题 $\tilde{P}$ 的优化模型具有如下形式，则称 $\tilde{P}$ 是 $P$ 的一个子问题 $$\min _{\tilde{x}_{1}} f_{1}\left(\tilde{x}_{1}\right) \quad \text { s.t. } \quad g_{i_{k}, 1}\left(\tilde{x}_{1} ; \beta_{i_{k}, 1}\right) \leq 0, \text { for some } i_{k} \in\{1, \cdots, N\}\tag{2}$$ 其中 $\beta_{i_k,j}$ 为参数，且建模 (2) 可以视为建模（1）的一部分
...
- 具体搜索过程如下：
	...
	2. 从根节点出发，在第一树层级的所有问题 $P_{1}^{(0)}, P_{2}^{(0)}, \cdots, P_{T}^{(0)}$ 中寻找第一个子问题：$$P^{(1)}=\arg \max _{P_{t}^{(0)}(t=1, \cdots, T)} I\left(P_{t}^{(0)} \subseteq_{S} P\right) \cdot \operatorname{Sim}_{\mathrm{LLM}}\left(C_{P_{t}^{(0)}}, C_{P}\right)$$ 其中 $I$ 为指示函数，条件为 True 取 1，否则取 $-\infin$。$\operatorname{Sim}_{\mathrm{LLM}}$ 表示由 LLM 给出的相似度分数，用于衡量两个问题之间的相似性
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/77eeee77cd9e4e77af7b9d3e58bb59f5.png#pic_left =80%x)
	1. **提取树搜索所需的陈述思路 $C_P$**
		* 时机：对数据集样本先跑树搜索/判定
		* 输入输出：问题描述 $\to C_P$
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/56edb75de74a4bafa8fa4f4d48d324aa.png#pic_left =80%x)
	1. **提取树搜索所需的陈述思路 $C_P$**
		* 时机：对新问题启动树搜索
		* 输入输出：问题描述 $\to C_P$
	2. **子问题判定 + 相似度打分（树搜索路径）**
...
- 在 7 个数据集上，使用 DeepSeek-V3 与 GPT-4o 评估 OptiTree，并与具有竞争力的基线比较
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/445c25398b6e43d5b0036f9cd238229e.png#pic_center =90%x)
...
- 下图显示了 OptiTree 的子问题分解质量
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/fef7f5618ac74bd7a8f4384f888d5815.png#pic_center#pic_center =95%x)
...
- 下图显示了 OptiTree 的时间开销，其中 Tree search 代表搜索建模树找到合适子问题的耗时，Modeling 表示结合 $T(P)$ 完成建模的耗时，Inference 表示求解问题总时间
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/82646243d3e04dcfb4e8bbb4bf375132.png#pic_center =85%x)
...
1. **树搜索是关键组件**：去掉树结构/树搜索（OptiTree w/o Tree Search）会明显降低建模准确率，说明树搜索能有效缩小搜索空间并减少幻觉
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/9a3d29591b61431f84b00d56483eb300.png#pic_center =60%x)

2. **建模思路 $T(P)$ 至关重要**：不使用建模思路（OptiTree w/o Modeling Thoughts）性能显著下降，且在更难数据集上降幅更大
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/8bb940fe51964e8dbd5fe73690e29297.png#pic_center =60%x)

3. **更深的搜索通常更好**：限制搜索深度会降低效果；深度越大越容易找到更合适的子问题分解，从而提升性能
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/817850e4c23a485c8c80ebb15babbcc4.png#pic_center =85%x)

4. **描述思路 $C_P$ 提升子问题匹配与整体性能**：移除 statement thoughts 会显著掉点，并导致子问题匹配准确率下降，说明其能提供更清晰的信息表示、降低幻觉
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/5d8c0e7f8d84450ab05fe7a18991f1f7.png#pic_center =85%x)
# 4. 总结
