---
title: 论文理解【LLM-Clarification】——【QDrawer】Asking the Right Question at the Right Time
date: 2026-02-05 20:56:22
index_img: /img/论文理解LLM_Clarification_QDrawerAskingtheRightQuestionattheRightTime/index.png
tags:
  - LLM-Clarification
  - LLM
categories:
...
- 首发链接：[论文理解【LLM-Clarification】——【QDrawer】Asking the Right Question at the Right Time](https://blog.csdn.net/wxc971231/article/details/157697050)
...
- 本文以 CoDraw 任务与数据集作为测试平台，研究协作式智能体中由**不确定性驱动的澄清策略**。CoDraw 是一个协作绘画游戏，涉及在**部分可观测**的虚拟环境中进行感知、交流和行动
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_Clarification_QDrawerAskingtheRightQuestionattheRightTime/img_001.png" alt="在这里插入图片描述" style="width: 60%;">
    </div>
    ...
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_Clarification_QDrawerAskingtheRightQuestionattheRightTime/img_002.png" alt="在这里插入图片描述" style="width: 90%;">
    </div>
...
- QDrawer 和 Silent Drawer 的主要区别在于交互方式不同，具体而言
    - Silent Drawer 每轮交互输入场景描述和画布状态 $(\text{Teller}_t, \text{Canvas}_t)$，输出绘制动作 $a_t$，即
        $$
        \begin{aligned}a_t &= \pi(\text{Teller}_t, \text{Canvas}_t) \\ \operatorname{Canvas}_{t+1}&=\operatorname{Apply}\left(\operatorname{Canvas}_{t}, a_{t}\right) \end{aligned}
        $$
        其中 $\text{Teller}_t$ 是数据集中的原始描述，遍历完数据样本的全部 $N$ 条描述即结束
    ...
    - QDrawer 外层也用 $\text{Teller}_t$ 遍历数据样本的全部 $N$ 条描述，但第 $t$ 个绘制动作 $a_t$ 生成前可能触发内层澄清问答，如下图所示
        <div align="center">
            <img src="/MyBlog/img/论文理解LLM_Clarification_QDrawerAskingtheRightQuestionattheRightTime/img_003.png" alt="在这里插入图片描述" style="width: 98%;">
        </div>
...
- 综上，QDrawer 方法的框架图如下
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_Clarification_QDrawerAskingtheRightQuestionattheRightTime/img_004.png" alt="在这里插入图片描述" style="width: 90%;">
    </div>
...
- 主实验结果如下，可见**基于内部不确定性发起提问的 QD-**$\theta$**实现了最高准确率，且澄清交流带来的增益显著高于基线模型**
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_Clarification_QDrawerAskingtheRightQuestionattheRightTime/img_005.png" alt="在这里插入图片描述" style="width: 60%;">
    </div>

- 作者进一步考虑了提出澄清问题的成本。首先注意到**可以用不确定度阈值控制提问数量**，从而间接地控制提问成本
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_Clarification_QDrawerAskingtheRightQuestionattheRightTime/img_006.png" alt="在这里插入图片描述" style="width: 60%;">
    </div>

    作者对不同提问数量（成本）对 size 精度的影响进行分析，发现**基于内部不确定性发起提问的 QD-**$\theta$**方法始终具有最高的性能提升效率**，无论进行几次澄清提问，始终具有优势。而**由人类数据学习到提问时机的性能和随机提问性能类似**
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_Clarification_QDrawerAskingtheRightQuestionattheRightTime/img_007.png" alt="在这里插入图片描述" style="width: 60%;">
    </div>
