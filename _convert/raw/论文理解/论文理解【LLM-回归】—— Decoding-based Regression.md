...
### 1.2.2 黎曼回归头（直方图分布）
- **`Riemann` 回归头使用了被广泛使用的分段常数（Riemann）基函数，其把数值范围划分成若干区间（bins） $\{y_1,...,y_n\}\sub\mathbb{R}$，模型输出的 logits 经过 softmax 转换为真实值落在各区间的概率，从而构成一个 “分布头”（distribution head）**
	$$p(y_i|x) = \text{Softmax}(i)\big(\phi(x)^T\cdot w\big)$$	其中 $\phi(x)$ 是编码器输出的样本 $x$ 的特征向量，$w$ 是可学习的分 bin 权重矩阵
