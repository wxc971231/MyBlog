...
- 下图显示了 Reflexion 框架在决策、编程和推理任务上的示例	
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/a384c5b6fef64a70a6692e885a003cdc.png#pic_center)
	1. 从任务描述开始
...
- 本节详细说明 Reflexion 框架的内部结构，如下图所示
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/d531f1d4eb5f4d0e9417e472bfa8b684.png#pic_center)
...
- Reflexion 和 Beam Search 机制有些类似，但是后者缺少 memory 组件，对比如下
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/483282e5497c4675afee631881dcf47b.png#pic_center =80%x)

# 2. 实验
- 在文字游戏任务 ALFWorld 上，Reflexion 机制使性能提升了 22%
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/4fd664c3997b4962945756dd7ac8e6ac.png#pic_center =90%x)
- 推理问答任务 HotPotQA 上，作者实现了一个 Reflexion + CoT 的 agent，它可以使用维基百科 API 检索上下文，并使用逐步显式思考推断答案。加入 Reflexion 使性能提升了 20%	
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/bc51bb0478ec42799328e88891c19024.png#pic_center =90%x)
- 编程任务上，由于涉及到编译器反馈，对比方法也或多或少地和使用了和 Reflexion 框架类似的组件或机制，比如生成单元测试，分析报错信息等，但是都没有 Reflexion 做得这么全面，且都没有核心的 Reflexion 机制
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/79476f31704f41d2afef0bc2829ebeee.png#pic_center =90%x)
	最终性能也是 Reflexion 显著更好
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/69dc334c8b294ff5bf29deb8840857fa.png#pic_center =90%x)
# 3. 总结
