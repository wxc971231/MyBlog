...
-  **`大规模的带可验证奖励的强化学习RLVR` 是在 “有自动可验证答案” 的任务上，对已训练大模型进行大规模 RL 微调的范式**。和 RLHF 使用单独训练的 Reward Mode 提供奖励不同，**RLVR 的奖励是自动算出来的**，比如数学题判断最终答案对不对、代码题跑单元测试是否通过等，这样就能低成本提供准确反馈
	> 形式化地讲：设有 LLM $\pi_\theta$，在 prompt $x\sim D$ 的条件下生成 token 序列 $y = (y_1,\ldots,y_T)$。一个确定性的验证器 $V$ 返回一个二值奖励：$r = V(x, y) \in \{0, 1\}$，且当且仅当模型的最终答案完全正确时 $r = 1$。通常还可以额外加入一个 *format reward*，以鼓励模型在输出中显式地将推理过程与最终答案分开。强化学习的目标是学习一个策略，以最大化期望奖励：$$J(\theta) = \mathbb{E}_{x \sim D} \mathbb{E}_{y \sim \pi_\theta(\cdot \mid x)}[r]$$ RLVR **形式上是一个标准的 RL 框架**：模型生成完整回答，判别器/验证器给一个 0/1 奖励（正确 / 错误），然后 PPO、GRPO、Reinforce++ 等各种 LLM-RL 算法来更新策略
...
- 作者使用 Pass@k 指标作为 LLM 推理能力边界的度量。该指标原本是代码生成领域的常用评估指标，作者将其推广到所有可验证奖励任务上。具体而言：给定一道题目，从模型中采样 $k$ 个输出，若其中至少有一个通过验证，则该题的 pass@k 取值为 1，否则为 0。对整个数据集取平均后的 pass@k 值就反映了**测试集中模型能在 $k$ 次尝试内能够解决的题目的比例**，从而对 LLM 的推理能力覆盖范围给出了一种严格的评估
	 > 在题目数、样本数都有限的情况下，直接计算 pass@k 的方差会很大，作者在此采用了一种低方差评估技巧：**对每题 $x_i$ 先抽 $n$ 个样本、数出其中正确个数 $c_i$，再用组合公式 $1-\binom{n-c_i}{k} /\binom{n}{k}$ 来得到这一题的 pass@k 概率**，这样得到的 pass@k 无偏估计是 $$\operatorname{pass@k}:=\mathbb{E}_{x_{i} \sim D}\left[1-\frac{\binom{n-c_{i}}{k}}{\binom{n}{k}}\right]$$ 这种估计方法**把 “随机选哪 k 个样本” 的那部分噪声给消掉了**，相当于做了一次 Rao–Blackwell 化，所以方差一定更小或相等。直观地看这一项：
...
- 作为一篇实证分析类工作，本文在数学、代码生成和视觉推理三个当前主流领域中，通过大规模实验对基座模型和 RLVR 模型进行了全面评估，实验设定如下
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/ff8efeb3a0f241f1b7de65c499979ad8.png#pic_center)
	...
	2. 采样模型答案时，统一采用温度 0.6、top-p 为 0.95，并允许最多生成 16,384 个 token
		> ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/f67b895d5f85419097dd6a3fdfff0031.png#pic_center =70%x)
...
- 实验结论：
	1. **RLVR 提高了数学推理任务正确样本概率，但降低可解问题覆盖率**，此结论对于 Oat-Zero 和 DAPO 等 RL 算法仍然成立
		> ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/5349a212ca854c67b5a24558e78a43ea.png#pic_center =80%x)
...
- 实验结论：和数学任务高度一致，**RLVR 提升了小 $k$ 值的平均性能，但大 $k$ 值时基座模型覆盖更广**
	> ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/56d56e7748e648708486caf11f6574ee.png#pic_center =50%x)
...
- 实验结论：
	1. 和数学、代码任务高度一致，**RLVR 提升了小 $k$ 值的平均性能，但大 $k$ 值时基座模型覆盖更广**
		> ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/73d358a0e5c04ad6a00b2b307539815c.png#pic_center =50%x)
...
- 第 2 节的实验表明，基座模型相比 RLVR 模型覆盖了更宽的可解问题范围。进一步分析 RL 训练前后准确率分布和可解问题覆盖率的差异，发现 **RLVR 在平均分上的提升并非源于解决了新的问题，而是来自于在原本就可由基座模型解出的那些问题上提高了采样效率**
	> ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/a35ffb793e1e4d38b787ac6be412aa9d.png#pic_center =90%x)
...
- 这引出一个自然问题：RL 训练模型生成的所有推理路径，是否都已存在于其基座模型的输出分布之中。为此作者使用 `困惑度perplexity` 这一度量进行分析
	> 给定模型 $m$，问题 $x$ 和一个回答序列 $Y=(y_1, ..., y_n)$，`困惑度`定义为该序列平均负对数似然的指数形式：$$\operatorname{PPL}_{m}(Y \mid x)=\exp \left(-\frac{1}{T} \sum_{t=1}^{T} \log P\left(y_{t} \mid x, y_{1}, \ldots, y_{t-1}\right)\right)$$ 它反映了在 prompt $x$ 条件下，模型对给定回答 $Y$ 的预测能力。困惑度越低模型生成该回答的可能性越高
	...
	> ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/4101514a5250448e974b3e838ee7b787.png)
...
- 作者发现：**从根本上受制于基座模型推理能力上限的 RL 不同，蒸馏可以从更强的教师模型中引入新的推理模式，从而超越基座模型的推理边界**
	...
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/f077475493b248df8de7ede2123defa5.png#pic_center =30%x)
## 3.3 不同 RL 算法的影响
- 根据实验，RL 的主要作用在于提升采样效率而非扩展模型的推理能力。作者提出 `采样效率差距”Sampling Efficiency Gap` 来量化对采样效率的提升，记作 $\Delta\text{SE}$
...
- 作者在 了多种主流 RL 算法，发现**各类现有 RL 方法在采样效率方面仍远未达到最优，要逼近上界可能需要全新的 RL 算法，甚至完全不同的范式**
	...
	![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/d4c06fd197d74f14b52a49ce6bd67458.png#pic_center =90%x)
...
1. **渐近效应**：随着 RL 训练的推进，训练集上的 pass@1 持续提升但 pass@256 会逐步下降，表明**推理边界在不断收缩**
	> ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/46b4db78b02248f583d252c8aa2e026a.png#pic_center =35%x)
2. Rollout 数量 $n$ 的影响：提升每个 prompt 生成的回答数量 $n$，可以扩大 RL 探索强度从而优化 pass@k，但 RL 模型**最终仍会被基座模型超越**
	> ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/39306a2e405e4d1e9005233f9c01f9ba.png)
...
- 现有研究已经发现：随着 RL 训练进行，模型输出的熵通常会降低，导致输出多样性减少并使推理边界收缩。为考察这一因素，作者提高 RLVR 训练模型的生成温度，使其输出熵与基座模型在 $T=0.6$ 时相匹配，发现**熵的降低确实促成了推理边界的收缩，但单靠这一因素并不足以完全解释边界缩小的现象**
	>  RLVR 模型在更高温度下的 pass@k 略有提升，但在各个 pass@k 上仍然不及基座模型
		![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/246bb3553b004f4589c968fcc32c0fd2.png#pic_center =85%x)
## 3.6 模型规模扩展的影响
...
- 由于多数大规模模型参数并未开源，作者使用 Magistral-Medium-2506 API 进行了一组初步实验。该模型以 Mistral-Medium-3-2505 作为起始模型，采用纯 RL 训练。Magistral-Medium 的表现与 DeepSeek-R1 相近，推理能力接近当前前沿水平。实验表明：**即便对于当前能力很强、接近前沿的推理模型，结论依然成立**
	> RLVR 在小 k 区间带来了显著增益，但在较大 k 时几乎没有提升![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b1b8938f87b3447a8b24879d4599af28.png#pic_center =75%x)
