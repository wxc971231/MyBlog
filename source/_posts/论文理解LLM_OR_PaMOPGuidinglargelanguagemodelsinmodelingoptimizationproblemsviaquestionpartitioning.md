---
title: 论文理解【LLM-OR】—— 【PaMOP】Guiding large language models in modeling optimization problems via question partitioning
date: 2026-01-25 20:15:14
index_img: img/论文理解LLM_OR_PaMOPGuidinglargelanguagemodelsinmodelingoptimizationproblemsviaquestionpartitioning/index.png
tags:
  - LLM-OR
  - LLM
categories:
  - 机器学习
  - 论文理解
description: 针对 LLM 求解运筹优化任务，PaMOP 通过约束划分树分离子问题，实现复杂优化问题的分块建模，降低子任务复杂度；结合求解器反馈与反向翻译一致性检查迭代纠错，实现建模逻辑错误识别与纠正，提升了建模准确性。
---

- 首发链接：[论文理解【LLM-OR】—— 【PaMOP】Guiding large language models in modeling optimization problems via question partitioning](https://blog.csdn.net/wxc971231/article/details/157328214)
...
- 本文研究**优化问题**的自动建模与编程，以减轻对人类专家的严重依赖。具体而言，这类问题要求**输入一段自然语言描述的问题**（如配送货、生产规划等问题），要求模型或系统完成**运筹学建模**，并**生成问题求解代码**。近期研究常借助 LLM 丰富的领域知识和推理能力完成建模
    ...
    > $$
    \max _{x} f(x) \quad \text { s.t. } \quad \forall i \in 1 . . n, g_{i}(x) \leq 0
    $$
    > 问题 $q$ 的数学模型可表示为 $M=\left(m_{p}, m_{v}, m_{o}, m_{c}\right)$，其中 $m_p$ 表示问题约束中的常量或参数，$m_v$ 表示决策变量 $x$，$m_o$ 表示目标函数 $f(x)$，而 $m_c$ 表示问题的约束 $g(x)$。这种四元组表示方法和 AMPL（以及 GAMS/OPL 这类代数建模语言）的核心组成高度一致，因此可以对应生成 AMPL 文件，再调用 gurobi 等求解器完成求解
...
# 2. 本文方法
 - 为了充分发挥 LLM 的建模推理能力，本文提出基于 LLMs 的自动化优化问题建模框架 PaMOP
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_OR_PaMOPGuidinglargelanguagemodelsinmodelingoptimizationproblemsviaquestionpartitioning/img_001.png" alt="在这里插入图片描述" style="width: 100%;">
    </div>
...
- PaMOP 使用 `约束集合constraint set` 从原始描述中得到相对独立的子问题，划分过程从包含全部问题信息的根节点开始，每层划分把节点拆分成若干子节点，**每个子节点内部的约束高度相关，不同子节点之间的约束相关性较弱**，划分得到的**叶节点代表需要建模的子问题**。具体而言，划分树的构建过程如下：
    ...
    3. **约束集合聚类**：仅靠独立集合分解难以有效降低问题难度，因为很多真实问题的约束会通过少量全局变量连起来，导致断不开或断得不细。因此，对于每个需要继续分解的节点，我们将其约束聚成若干集合，从而生成新的子节点，每层划分目标是**让子节点内约束更相关、子节点间相关性更弱**（论文未强调约束集合严格不重叠，且聚类噪声点会被保留）。重复进行 “聚类—生成子节点” 的过程，直到节点已足够简单或无法继续有效划分，由此得到的叶节点将作为后续建模的子单元
        > 为进行聚类，约束之间距离定义为两条约束相似度 $s_{i, j}$ 的反函数
        > $$
        d_{i, j}=\frac{1}{s_{i, j}+\varepsilon}-\frac{1}{1+\varepsilon}
        $$
        > 其中 $\varepsilon$ 是一个固定小值，如下图所示，这种距离方式可以使 $s_{i, j} \to 0$ 时约束间距离迅速增大
        > <div align="center">
        >     <img src="/MyBlog/img/论文理解LLM_OR_PaMOPGuidinglargelanguagemodelsinmodelingoptimizationproblemsviaquestionpartitioning/img_002.png" alt="在这里插入图片描述" style="width: 70%;">
        > </div>
...
- 一个示例如下，原始问题由问题描述和数据文件组成（AMPL格式特点，建模可以和数据分离），仅靠 LLM CoT 推理常遗漏约束，而 PaMOP 可以正确完成建模
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_OR_PaMOPGuidinglargelanguagemodelsinmodelingoptimizationproblemsviaquestionpartitioning/img_003.png" alt="在这里插入图片描述" style="width: 80%;">
    </div>

    下图是问题划分树的示例
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_OR_PaMOPGuidinglargelanguagemodelsinmodelingoptimizationproblemsviaquestionpartitioning/img_004.png" alt="在这里插入图片描述" style="width: 80%;">
    </div>
...
- 对第 $i$ 个叶节点对应的局部模型 $m_i$ 进行建模时，需要该节点的约束描述 $t_{c}$、变量信息 $t_v$ 和全局摘要 $g$，以确保 LLM 在建模时保持对整体问题的认识，表示为
    $$
    m_{c, i}=G_{\bmod }\left(g, t_{v}, \bigcup_{j \in \operatorname{cons}_{i}} t_{c, j}\right)
    $$
    其中 $\operatorname{cons}_{i}$ 表示节点 $i$ 所包含的约束列表，$t_{c,j}$ 表示 $t_c$ 中第 $j$ 条约束（包含文本和模糊度分数）
...
- NLP4LP 和真实世界问题上的性能如下
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_OR_PaMOPGuidinglargelanguagemodelsinmodelingoptimizationproblemsviaquestionpartitioning/img_005.png" alt="在这里插入图片描述" style="width: 95%;">
    </div>
...
- 使用 GPT4 模型，从简单的纯 prompt baseline 开始，逐步加入 PaMOP 组件
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_OR_PaMOPGuidinglargelanguagemodelsinmodelingoptimizationproblemsviaquestionpartitioning/img_006.png" alt="在这里插入图片描述" style="width: 90%;">
    </div>
...
- 对比了使用 GPT-3.5-turbo、GPT-4 与 Llama-3.3-70b 作为基座模型的效果
    <div align="center">
        <img src="/MyBlog/img/论文理解LLM_OR_PaMOPGuidinglargelanguagemodelsinmodelingoptimizationproblemsviaquestionpartitioning/img_007.png" alt="在这里插入图片描述" style="width: 90%;">
    </div>
